...
if(isset($_POST['submit'])) {
    ...
    if (!empty($fname) && !empty($uname) && !empty($email) && !empty($pnumber) && !empty($password)) {
        ...
        if($email == $db_email){
            ...
            $query = "INSERT INTO users (fname, uname, referral, reflink, email, pnumber, password, token, user_image) VALUES ('{$fname}', '{$uname}', '{$referral}', '{$reflink}', '{$email}', '{$pnumber}', '{$password}', '{$token}', '0')";
